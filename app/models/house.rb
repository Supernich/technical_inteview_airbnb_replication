...
class House < ApplicationRecord
  ...
  def last_check_out_date
    house_bookings.from_check_out_date(Date.current).order(check_out: :desc).first&.check_out
  end

  def first_check_in_date
    house_bookings.from_check_in_date(Date.current).order(check_in: :asc).first&.check_in
  end
end
