...
RSpec.describe HousesSearchService do
  ...
  describe '#filter_houses' do
    it 'returns filtered houses by provided country' do
      expect(described_class.filter_houses({ country: 'country1' }, House.all).as_json(only: :name))
        .to(eq([{ 'name' => 'house1' }, { 'name' => 'house2' }]))
    end

    it 'returns filtered houses by provided country even if country name not full' do
      expect(described_class.filter_houses({ country: 'try1' }, House.all).as_json(only: :name))
        .to(eq([{ 'name' => 'house1' }, { 'name' => 'house2' }]))
    end

    it 'returns filtered houses by provided city' do
      expect(described_class.filter_houses({ city: 'city2' }, House.all).as_json(only: :name))
        .to(eq([{ 'name' => 'house2' }, { 'name' => 'house3' }]))
    end

    it 'returns filtered houses by provided city even if city name not full' do
      expect(described_class.filter_houses({ city: 'ty2' }, House.all).as_json(only: :name))
        .to(eq([{ 'name' => 'house2' }, { 'name' => 'house3' }]))
    end

    it 'returns filtered houses by provided check_in date' do
      expect(described_class.filter_houses({ check_in: Date.new(2022, 3, 24) }, House.all).as_json(only: :name))
        .to(eq([{ 'name' => 'house3' }, { 'name' => 'house4' }]))
    end

    it 'returns filtered houses by provided check_out date' do
      expect(described_class.filter_houses({ check_out: Date.new(2022, 3, 29) }, House.all).as_json(only: :name))
        .to(eq([{ 'name' => 'house3' }, { 'name' => 'house4' }]))
    end
